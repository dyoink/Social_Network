      ...
      {!isAuthenticated ? (
        ...
        <>
          <TopNav 
            ...
            setView={setView} 
            onProfileClick={() => handleViewProfile(currentUser || MOCK_USER)} 
          ...
          <nav className="md:hidden fixed bottom-0 w-full bg-white/90 backdrop-blur-xl border-t border-surface-container flex justify-around items-center h-16 px-4 z-50">
            ...
            <button onClick={() => handleViewProfile(currentUser || MOCK_USER)} className={`p-2 ${currentView === 'profile' && selectedUser.id === (currentUser?.id || MOCK_USER.id) ? 'text-primary' : 'text-outline'}`}><User className="w-6 h-6" /></button>
          </nav>
        </>
      )}
    </div>
